(function () {
    ...
    var PREFIX = location.href.indexOf("/web.html") != -1 ? "https://play.rust-lang.org/" : "https://play.rust-lang.org/";
    ...
    function redrawResult(result) {
        ...
        // so we want to manually trigger a redraw. It doesnâ€™t matter
        // whether itâ€™s relative or static for now, so weâ€™ll flip that.
    ...
    function evaluate(result, code, version, optimize, button, test, backtrace) {
        ...
        }, button, test ? "Running testsâ€¦" : "Runningâ€¦", result);
    }
    ...
    function repaintResult() {
        ...
        // least Firefox, so we want to manually trigger a repaint. It doesnâ€™t
        // matter whether itâ€™s relative or static for now, so weâ€™ll flip that.
        result.parentNode.style.visibility = "hidden";
        var _ = result.parentNode.offsetHeight;  // This empty assignment is intentional
        result.parentNode.style.visibility = "";
    }
    ...
    function share(result, version, code, button, backtraceval) {
        ...
        if (playurl.length > 5000) {
            set_result(result, "<p class=error>Sorry, your code is too long to share this way." +
                ...
                " code in the URL, and the URL shortener used doesnâ€™t accept URLs longer than" +
    ...
    function updateEvaluateAction(code) {
        // A very simple pair of heuristics; thereâ€™s no point in doing more, IMO.
    ...
    addEventListener("DOMContentLoaded", function() {
        ...
        // Weâ€™re all pretty much agreed that such an obscure command as transposing
        // letters hogging Ctrl-T, normally â€œopen new tabâ€, is a bad thing.
